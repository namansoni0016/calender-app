...
export default async function BookEventPage({ params: { clerkUserId, eventId }}: { params: { clerkUserId: string, eventId: string }}) {
    ...
    const calenderUser = await clerkClient().users.getUser(clerkUserId);
    ...
    if(validTimes.length === 0) {
        return <NoTimeSlots event={event} calenderUser={calenderUser} />
    }
    return (
        <Card className="max-w-4xl mx-auto">
            <CardHeader>
                <CardTitle>
                    Book {event.name} with {calenderUser.fullName}
                </CardTitle>
                {event.description && (
...
function NoTimeSlots({ event, calenderUser }: { 
    event: { name: string; description: string | null}
    calenderUser: { id: string, fullName: string | null }
}) {
    return (
        <Card className="max-w-md mx-auto">
            ...
            <CardContent>
                {calenderUser.fullName} is currently booked up. Please check back later or choose a shorter event.
            </CardContent>
            <CardFooter>
                <Button asChild>
                    <Link href={`/book/${calenderUser.id}`}>
