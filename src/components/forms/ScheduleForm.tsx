...
export function ScheduleForm({ schedule }: { schedule?: {
    ...
    const groupedAvailabilityFields = Object.groupBy(availabilityFields.map((field, index) => ({ ...field, index })), availability => availability.dayOfWeek);
    async function onSubmit(values: z.infer<typeof scheduleFormSchema>) {
        ...
        if(data?.error) {
            ...
            setSuccesMessage("Schedule saved!");
        }
    }
    return (
        <Form {...form}>
                ...
                <div className="grid grid-cols-[auto,1fr] gap-y-6 gap-x-4">
                    {DAYS_OF_WEEK_IN_ORDER.map(dayOfWeek => (
                        <Fragment key={dayOfWeek}>
                            <div className="capitalized text-sm font-semibold">{dayOfWeek.substring(0, 3)}</div>
